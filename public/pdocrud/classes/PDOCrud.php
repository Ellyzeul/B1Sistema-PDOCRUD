...
Class PDOCrud {
    ...
    * @return   object                                         Object of class
    ...
    private function initializeHTMLContent() {
        $this->HTMLContent[] = $this->getAjaxLoaderImage($this->settings["script_url"] . "images/ajax-loader.gif");
        return $this;
    }
    ...
    private function enqueuePlugin() {
        $pluginPath = $this->settings["script_url"] . "plugins/";
    ...
    private function registerJs() {
        ...
        foreach ($loadInitialJs as $js) {
            $jsLists[$js] = $scripturl . "js/$js";
        }
        if (!isset($this->settings["includeTemplateJS"]) || (isset($this->settings["includeTemplateJS"]) && $this->settings["includeTemplateJS"])) {
            $jsLists["template-script"] = $scripturl . "classes/templates/" . $templateName . "/js/script.js";
        } 
        $jsLists["pdocrud-script"] = $scripturl . "js/comman.js";
    ...
    public function setSkin($skin) {
        ...
        if (is_array($skin)) {
            foreach ($skin as $sk) {
                $cssPath = $scripturl . "skin/" . $sk . ".css";
                $this->enqueueCss($sk, $cssPath);
            }
        } else {
            $cssPath = $scripturl . "skin/" . $skin . ".css";
    ...
    private function registerCss() {
        ...
        foreach ($loadInitialCss as $css) {
            $cssLists[$css] = $this->settings["script_url"] . "css/$css";
        }
        
        if (!isset($this->settings["includeTemplateCSS"]) || (isset($this->settings["includeTemplateCSS"]) && $this->settings["includeTemplateCSS"])) {
            $cssLists["template-css"] = $scripturl . "classes/templates/" . $templateName . "/css/style.css";
        }
...
//        unset($this->btnActions["edit"]);
    ...
    public function getCaptcha($fieldName) {
        ...
        $imageSrc = $this->settings["script_url"] . "/classes/library/captchamath/CaptchaMath.php?objId=$this->formId";
