...
const fields = {
  ...
  deadline: {isDate: true, editable: false, label: "Prazo para retirada"},
...
const updateRow = (trackingCode: string, deliveryMethod: string, row: any, fields: {[key: string]: {editable: boolean, label: string}}) => {
  api.post('/api/tracking/update-purchase', {
    tracking_code: trackingCode,
    ...
    .catch(error => {
      ...
      else toast.error("Ocorreu algum erro... Entrar em contato com o setor de TI")
    })
}
...
const getRows = (data: {[key: string]: string}[], fieldsKeys: string[], actualPage: number) => {
  ...
  data.slice(offset, offset + ROWS_PER_PAGE).forEach((row, idx) => {
    const btnCell = <td 
      className="purchase-tracking-update-button" 
...
export const PurchaseTrackingTable = (props: TrackingTableProp) => {
    ...
    api.post('/api/tracking/read-for-excel', {
      ...
      .then(response => {
        ...
        workbook.xlsx.writeBuffer()
          ...
          .then(blob => new File([blob], 'Rastreamento_compras.xlsx'))
          .then(file => {
            const url = URL.createObjectURL(file)
            const anchor = document.createElement('a')
            anchor.href = url
            anchor.download = "Rastreamento_compras.xlsx"
            anchor.click()
    ...
    <div className="tracking-table-container">
      <div className="tracking-nav-buttons">
        ...
        <button className="tracking-download-xlsx" onClick={downloadExcel}>
          ...
          <button onClick={updateAllTrackings} className="purchase-tracking-update-button purchase-tracking-updateall-button">Atualizar todos</button>
        <div>
          <span>Página </span>
...
export default PurchaseTrackingTable
